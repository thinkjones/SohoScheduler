...
jQuery(function($){
    ...
    var namingConventionTypes = {
        ...
        'DM':['se','ch','rq','pid','cid'],   // Design time Misc CheckBox Control
	...
	// Public Functions
	// -------------------------------------------------------
	$.fn.sohoFormBuilder = function(options) {

        //Merged passed and default options
        $.fn.sohoFormBuilderOptions(options);
    ...
    $.fn.sohoFormBuilderOptions =  function(options) {
    ...
    $.fn.sohoFormLoadDataByControl = function(calling_control, pkid, mnemonic) {
    ...
    function RenderClass(ul_obj, options){
        ...
        renderRuntime= function(){
            ...
            for(var control in jsonDict){
                control_attrs = jsonDict[control];
                var htmlDT = document.createElement("div");
                var htmlDD = document.createElement("div");
                ...
                $(htmlDD).append(htmlDIVControl).append(htmlLabel);
        ...
        createComplexControls = function(id,control_attrs){
            ...
            if(control_attrs.displayType=='date'){
                ...
                var anchorHandle = document.createElement('a');
                var anchorID = $(ctrlJQ).attr('id') + 'chooser';
                //$(anchorHandle).attr('class','choose-date').attr('href','#').attr('id',anchorID);
                //$(anchorHandle).click(function(){$(ctrlJQ).focus();return false;})
                //$(ctrlJQ).parent().append(anchorHandle);

                //Add Timepicker ANchors
                $('#' + ctrlID + '_tf').timepickr({convention: 12});
                $('#' + ctrlID + '_tt').timepickr({convention: 12});

                $('#' + ctrlID + '_df').attr('value_field_name',control_attrs.label + '||df');
                $('#' + ctrlID + '_tf').attr('value_field_name',control_attrs.label + '||tf');
                $('#' + ctrlID + '_tt').attr('value_field_name',control_attrs.label + '||tt');
                $('#' + ctrlID + '_dt').attr('value_field_name',control_attrs.label + '||dt');
        ...
        renderRuntimeControlIntoPlaceholder = function(id,controlType,bolCreateRuntimeInDesignMode,control_attrs){
            ...
            //Control Label
            var htmlControlLabel = document.createElement("h3");
            $(htmlControlLabel).attr('id', GetObjectID('RL',id)).text(control_attrs.label);

            //Required Label
            var htmlReq = document.createElement("h4");
            ...
            var htmlDT = document.createElement("div");
            $(htmlDT).append(htmlControlLabel).append(htmlReq).addClass('field-title');

            //Create right hand side container
            var htmlDD = document.createElement("div");
            $(htmlDD).append(htmlDIVControl).append(htmlHelpText).addClass('field-control');
            ...
            if(bolCreateRuntimeInDesignMode){
                ...
                if (controlType == 'date') {
                        var strTimerequired = $(GetJQObjectID('DM', id,'0'));
                        $(strTimerequired).change(Date_OnClick);
                        var strRangerequired = $(GetJQObjectID('DM', id,'1'));
                        $(strRangerequired).change(Date_OnClick);
                }
            }
        };
        ...
        Date_OnClick = function(){
            var parent_id = GetParentIDForControl(this);
            var control_attrs = getfieldAttributesJSON(parent_id);
            renderRuntimeDateControl(parent_id,control_attrs.show_date_range, control_attrs.show_time_picker);
        }

        renderRuntimeDateControl = function(id, show_date_range, show_time_picker){
            //Get runtime wrapper div
            var divRuntime = GetJQObjectID('RW',id);
            var date_elements = $(divRuntime + ' ul.daterange-picker>li');
            
            //There should be 4 date elements, df, tf, tt, dt
            $(date_elements[0]).show();
            $(date_elements[1]).hide();
            $(date_elements[2]).hide();
            $(date_elements[3]).hide();

            if(show_date_range && show_time_picker){
                $(date_elements[1]).show();
                $(date_elements[2]).show();
                $(date_elements[3]).show();
            }
        ...
        this.loadControlWithData = function(field_name, dataDict){
            ...
            var sub_field_type = dataDict['sub_field_type'];
            ...
            if(sub_field_type){
                inputID = inputID + '_' + sub_field_type;
            }
        ...
        create_FB_RuntimeControl = function(id,controlType,control_attrs){
            var htmlControl = null;
            
            if(controlType=='date'){
                htmlControl = create_FB_RuntimeControl_Date(id,controlType,control_attrs);
            }else{
                htmlControl = create_FB_RuntimeControlStandard(id,controlType,control_attrs);
            }
            
            return htmlControl;
        };

        create_FB_RuntimeControlStandard= function(id,controlType,control_attrs){
            ...
            var htmlControlID = GetObjectID('RW',control_attrs.id);
            $(htmlControl).attr('id',htmlControlID);
        ...
        create_FB_RuntimeControl_Date = function(id,controlType,control_attrs){
            ...
            var htmlControls = [];
            var htmlControlAnchors = [];

            //From Date
            htmlControls[0] = $(htmlControl).clone();
            $(htmlControls[0]).attr('id',htmlControlID + '_df').datepicker({changeMonth: true,changeYear: true})
            htmlControlAnchors[0] = inputHandle(htmlControlID + '_df_chooser','choose-date');

            //From Time
            htmlControls[1] = $(htmlControl).clone();
            $(htmlControls[1]).attr('id',htmlControlID + '_tf')
            htmlControlAnchors[1] = inputHandle(htmlControlID + '_tf_chooser','choose-time');

            //From To Range Label
            var htmlRangeLabelControl = document.createElement('span');
            $(htmlRangeLabelControl).text(' to ').addClass('floatleft');


            //ToTime
            htmlControls[2] = $(htmlControl).clone();
            $(htmlControls[2]).attr('id',htmlControlID + '_tt')
            htmlControlAnchors[2] = inputHandle(htmlControlID + '_tt_chooser','choose-time');

            //To Date
            htmlControls[3] = $(htmlControl).clone();
            $(htmlControls[3]).attr('id',htmlControlID + '_dt').datepicker({changeMonth: true,changeYear: true});
            htmlControlAnchors[3] = inputHandle(htmlControlID + '_dt_chooser','choose-date');

            
            htmlControl = document.createElement('div');
            $(htmlControl).attr('id',htmlControlID);

            var ulDateWrapper = document.createElement('ul');
            $(ulDateWrapper).addClass('daterange-picker')
            var iIndex = 0;
            for(iIndex=0;iIndex<htmlControls.length;iIndex++){
                var liDateWrapper = document.createElement('li');
                $(liDateWrapper).append(htmlControls[iIndex]);  //not using anchors at the moment too much trouble.append(htmlControlAnchors[iIndex]);
                $(ulDateWrapper).append(liDateWrapper);
            }
            $(htmlControl).append(ulDateWrapper)

            return $(htmlControl);
        };

        inputHandle = function(base_id,anchor_class){
            //Add Date Anchors
            var anchorHandle = document.createElement('a');
            var anchorID = base_id + 'chooser';
            $(anchorHandle).attr('class',anchor_class).attr('href','#').attr('id',anchorID);
            return $(anchorHandle);            
        };
        ...
        create_FB_ElementsDiv = function(id, ctrlType,controlAttributes){
            ...
            var fbHelpText;
            var fbCtrlName 		=  create_FB_Input(GetObjectID('DL',id), 'text', '').addClass('options-input-standard');
			
            if(typeof(ctrlType) != 'undefined' && ctrlType == 'textarea'){
                fbHelpText 		=  create_FB_TextArea(GetObjectID('DH',id), 'text', '').addClass('options-input-standard');
            }else{
                fbHelpText 		=  create_FB_Input(GetObjectID('DH',id), 'text', '').addClass('options-input-standard');
            }
            ...
            var htmlDL = document.createElement('table');
            ...
            if(ctrlType=='date'){
            var id_fbIncludeTimeCheckbox = GetObjectID('DM',id,'0');
                var fbIncludeTimeCheckboxLabel =  create_FB_Label().addClass('fb-required-label').attr('for',id_fbIncludeTimeCheckbox).text('Show Time Picker');
                var fbIncludeTimeCheckbox =  create_FB_Input(id_fbIncludeTimeCheckbox, 'checkbox', 1).addClass('checkbox');
                if (typeof(controlAttributes.show_time_picker) != 'undefined') {
                    if(controlAttributes.show_time_picker == 'true' || controlAttributes.show_time_picker == true)
                        fbIncludeTimeCheckbox.attr('checked',true);
                }
                var rowIncludeTime = create_FB_Attribute_Row(fbIncludeTimeCheckboxLabel,fbIncludeTimeCheckbox);

                var id_fbIncludeDateRangeCheckbox = GetObjectID('DM',id,'1');
                var fbIncludeDateRangeCheckboxLabel =  create_FB_Label().addClass('fb-required-label').attr('for',id_fbIncludeDateRangeCheckbox).text('Show Date Range');
                var fbIncludeDateRangeCheckbox =  create_FB_Input(id_fbIncludeDateRangeCheckbox, 'checkbox', 1).addClass('checkbox');
                if (typeof(controlAttributes.show_date_range) != 'undefined') {
                    if(controlAttributes.show_date_range == 'true' || controlAttributes.show_date_range == true)
                        fbIncludeDateRangeCheckbox.attr('checked',true);
                }
                var rowIncludeDateRange = create_FB_Attribute_Row(fbIncludeDateRangeCheckboxLabel,fbIncludeDateRangeCheckbox);

                $(jqDL).append(rowIncludeTime).append(rowIncludeDateRange);
            }
        ...
        GetParentIDForControl= function(childControl){
            return GetControlParentWrapperID(childControl,'uid','','uid','uid');
        };
        ...
        create_FB_Attribute_Row = function(dt,dd){
            var divHolder = document.createElement("tr");
            var htmlDT = document.createElement("td");
            var htmlDD = document.createElement("td");
            $(htmlDT).append(dt);
            $(htmlDD).append(dd);
        ...
        getfieldAttributesJSON = function(liid,posIndex) {
            ...
            var liRow = $(strli)[0];
             ...
             //uid
               ...
               // Get Date speciifc Values
               var show_date_range = false;
               var show_time_picker = false;
               if(controlType=='date'){
                   var jqRangerequired = $(GetJQObjectID('DM', uid,'1'));
                   var jqTimerequired = $(GetJQObjectID('DM', uid,'0'));
                   show_date_range = ($(jqRangerequired).attr("checked") ? $(jqRangerequired).val() : 0);
                   show_time_picker = ($(jqTimerequired).attr("checked") ? $(jqTimerequired).val() : 0);
               }
