...
(function($) {
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		for (var i = 0; i < _totalBoxes; i++) {
			...
			var dateLink = jQuery('<div class="DateLabel"><a href="">' + currentDate.getDate() + '</a></div>').click(function(e) {
                thisDate = jQuery.J.CreateDateFromAttribute($(this).attr('date'));
				defaults.onDayLinkClick(thisDate);
				e.stopPropagation();
			});
			
			var dateBox = jQuery("<td></td>").attr(atts).append(dateLink).dblclick(function(e) {
                thisDate = jQuery.J.CreateDateFromAttribute($(this).attr('date'));
				defaults.onDayCellDblClick(thisDate);
				e.stopPropagation();
			}).click(function(e) {
                thisDate = jQuery.J.CreateDateFromAttribute($(this).attr('date'));
				defaults.onDayCellClick(thisDate);
				e.stopPropagation();
			});
    ...
    jQuery.J.CreateDateFromAttribute = function(attr_date) {
        dteComponents = attr_date.split('/');
        intMonth = parseInt(dteComponents[0],0)-1;
        if(intMonth==12)
            intMonth = 0;
        if(intMonth==-1)
            intMonth = 11;
        dteRet = new Date(dteComponents[2],intMonth,dteComponents[1]);
        return dteRet;
	}
