...
(function($){
	$.fn.formbuilder = function(options) {
		...
		var defaults = {
            ...
            toolbox_control: 'ul',
            controls: {
                'input_text':{'DisplayName':'Text','ControlType':'input_text'},
                'textarea':{'DisplayName':'Paragraph','ControlType':'textarea'},
                'checkbox':{'DisplayName':'Checkboxes','ControlType':'checkbox'},
                'radio':{'DisplayName':'Radio','ControlType':'radio'},
                'select':{'DisplayName':'Select List','ControlType':'select'}
            }
		...
		return this.each(function() {
            ...
            for (var listitem in opts.controls){
                var objList = document.createElement(list_control);
                var listitemValue = opts.controls[listitem]['ControlType'];
                $(objList).attr('ControlType',listitemValue).text(opts.controls[listitem]['DisplayName']);  //.text(listitem['DisplayName']);
                $(objSelect).append(objList);
            }

            $(objSelect).attr('class','field_control toolbar');
            var objTopSelect = objSelect.cloneNode(true);
            var objBotSelect = objSelect.cloneNode(true);
            $(objTopSelect).attr('name','field_control_top');
            $(objBotSelect).attr('name','field_control_bot');
            var objTopDiv = document.createElement('div');
            $(objTopDiv).append(objTopSelect);
            var objBottomDiv = document.createElement('div');
            $(objBottomDiv).append(objBotSelect);

            $(this).before($(objTopDiv).html() + hr_clearboth);
            $(this).after($(objBottomDiv).html() + hr_clearboth);

            var function_select_item = function(){
				//appendNewField($(this).val());
				appendNewField($(this).attr('ControlType'));
				$(this).val(0).blur();
				$().scrollTo($('#frm-'+(last_id-1)+'-item'), 800);
				return false;
            }

			//$('.field_control').change(function_select_item);
			$('.field_control>li').click(function_select_item);
